...
if sys.argv[0] != 'Gaussian.py':
    ...
    args = parser.parse_args(['--device', '0',
                              ...
                              '--std', '2',
                              '--iTest', '55', '58',
                              '--outFile', '/home/dwu/trainData/Noise2Noise/train/ctp/simul/gaussian/test'])
else:
...
if args.std > 0:
    reconMask = scipy.ndimage.filters.gaussian_filter1d(mask.astype(np.float32), args.std, 1)
    reconMask = scipy.ndimage.filters.gaussian_filter1d(reconMask, args.std, 2)

    recon = scipy.ndimage.filters.gaussian_filter1d(imgs, args.std, 1)
    recon = scipy.ndimage.filters.gaussian_filter1d(recon, args.std, 2)

    reconMask[reconMask <= 1e-6] = 1e-6
    recon = recon / reconMask[...,np.newaxis]
    recon = recon - (recon[..., [0]] + recon[..., [1]]) / 2
else:
    recon = imgs - (imgs[...,[0]] + imgs[...,[1]]) / 2
